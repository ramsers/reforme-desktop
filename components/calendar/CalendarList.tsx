...
type CalendarListProps = CalendarListOwnProps &
    CalendarListSliceProps &
    CalendarListDispatchProps

const CalendarList: React.FC<CalendarListProps> = ({items, emptyMessage = ""}) => {
    if (!items || items.length === 0) {
        return (
            <div className="text-gray-500 text-center py-10">{emptyMessage}</div>
        );
    }
    ...
    return (
        <div className="w-full max-w-2xl flex flex-col gap-3">
            {items && items?.map((item) => (
                <div key={item.id} className="p-4 rounded-xl shadow bg-white border border-gray-200">
                    <div className="flex flex-row items-center justify-between">
                        <div className="flex flex-col gap-1">
                            <div className="text-lg font-bold">{item.title}</div>
                            <div className="text-sm text-gray-600">{item.description}</div>
                            <span className="text-sm">{item.instructorName} • {dayjs(item.date).format("h:mm A")}</span>
                        </div>
